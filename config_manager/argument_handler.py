...
def parse_arguments() -> dict[str, Union[str, int, bool]]:
    ...
    """
    argument_dict: dict[str, Union[str, int, bool]] = {
    ...
    if not host_ip_value:
        bind_ip = argument_dict["BIND_IP"]
        if bind_ip != '0.0.0.0':
            host_ip_value = str(bind_ip)
    if not host_ip_value:
        fallback = get_ip_address()
        host_ip_value = fallback if fallback else '0.0.0.0'
    argument_dict["HOST_IP"] = host_ip_value
    ...
    argument_dict["HTTP_PORT"] = http_port_value if http_port_value else 5002
    argument_dict["RUNNING_PATH"] = str(pathlib.Path(__file__).parent.parent)
